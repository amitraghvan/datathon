...
def generate_markdown_report(audit_data: Dict[str, Any], output_path: Path | None = None) -> Path:
    ...
    md.append(
        f"- **CRITICAL**: {sev_counts.get('CRITICAL', 0)} (Impossible attendance, proxy marking fraud, missing quantities)"
    )
    md.append(
        f"- **HIGH**: {sev_counts.get('HIGH', 0)} (Mixed grading scales, messy booleans, duplicate master records, currency formatting)"
    )
    md.append(f"- **MEDIUM**: {sev_counts.get('MEDIUM', 0)} (Missing primary keys / record IDs)")
    md.append(
        f"- **INFO**: {sev_counts.get('INFO', 0)} (100% Referential integrity verified post-canonicalization)"
    )
    ...
    for child, stats in ref_int.items():
        status = (
            "PASSED (100% Match)"
            if stats["integrity_passed"]
            else f"FAILED ({stats['unmatched_schools']} orphans)"
        )
        md.append(
