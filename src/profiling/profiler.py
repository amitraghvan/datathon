...
def profile_dataframe(df: pd.DataFrame, dataset_name: str, key_col: str | None = None) -> Dict[str, Any]:
...
def run_comprehensive_audit() -> Dict[str, Any]:
    ...
    profiles = {
        ...
        "student_attendance": profile_dataframe(df_att, "track4_student_attendance.csv", "record_id"),
        "school_infrastructure": profile_dataframe(df_infra, "track4_school_infrastructure.csv", "inspection_id"),
        "mdm_procurement": profile_dataframe(df_mdm, "track4_mid_day_meal_procurement.xlsx", "procurement_id"),
    ...
    amenity_cols = ["has_electricity", "has_drinking_water", "has_functional_toilet", "has_boundary_wall", "has_playground"]
    ...
    audit_result = {
        ...
        "issues": issues,
        "issue_severity_counts": pd.Series([i["severity"] for i in issues]).value_counts().to_dict(),
    }
